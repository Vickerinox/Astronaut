...
pub mod fatfs {
    ...
    impl TryFrom<i32> for Error {
        ...
        fn try_from(v: i32) -> Result<Self, Self::Error> {
            match v {
                x if x == Error::DiskError as i32 => Ok(Error::DiskError),
                x if x == Error::IntError as i32 => Ok(Error::IntError),
                x if x == Error::NotReady as i32 => Ok(Error::NotReady),
                x if x == Error::NoFile as i32 => Ok(Error::NoFile),
                x if x == Error::NoPath as i32 => Ok(Error::NoPath),
                x if x == Error::InvalidName as i32 => Ok(Error::InvalidName),
                x if x == Error::Denied as i32 => Ok(Error::Denied),
                x if x == Error::Exists as i32 => Ok(Error::Exists),
                x if x == Error::InvalidObject as i32 => Ok(Error::InvalidObject),
                x if x == Error::WriteProtected as i32 => Ok(Error::WriteProtected),
                x if x == Error::InvalidDrive as i32 => Ok(Error::InvalidDrive),
                x if x == Error::NotEnabled as i32 => Ok(Error::NotEnabled),
                x if x == Error::NoFileSystem as i32 => Ok(Error::NoFileSystem),
                x if x == Error::MkfsAborted as i32 => Ok(Error::MkfsAborted),
                x if x == Error::Timeout as i32 => Ok(Error::Timeout),
                x if x == Error::Locked as i32 => Ok(Error::Locked),
                x if x == Error::NotEnoughCore as i32 => Ok(Error::NotEnoughCore),
                x if x == Error::TooManyOpenFiles as i32 => Ok(Error::TooManyOpenFiles),
                x if x == Error::InvalidParameter as i32 => Ok(Error::InvalidParameter),
                _ => Err(()),
            }
        }
    }
