...
fn apply_patch() {
    ...
    let mut l_words: Vec<_> = launcherd
        .chunks_exact(2)
    ...
    println!("result: {a:?}");
    let mut expected_words: Vec<u16> =
        include_bytes!("/home/vik/Documents/NO$GBA/SLOT/patched_launcher.bin")
            .chunks_exact(2)
            .map(|i| u16::from_le_bytes([i[0], i[1]]))
            .collect();
...
fn app_unlaunch_patch(l_words: &mut [u16], patch: Vec<UnlaunchBlockPatch>) {
    ...
    for (i, patch) in patch.iter().enumerate() {
        ...
        loop {
            while l_words[l_cursor] != patch.original_fn[0] {
                l_cursor += 1
            }
            let match_length = l_words[l_cursor..]
                .iter()
                .zip(patch.original_fn.iter())
                .filter(|(a, b)| (**b == 0xAAAA) || (**b == **a))
                .count();
...
fn translate_patch(patches: &[UnlaunchBlockPatch]) -> (Vec<VBlock>, Vec<u16>, Vec<u16>) {
    ...
    for patch in patches {
        blocks.push(VBlock {
            original_len: patch.original_fn.len() as _,
            patch_len: patch.patch_block.len() as _,
            offset: patch.patch_block_offset,
        });
...
fn decode_patch(mut as_words: impl Iterator<Item = u16>) -> Vec<UnlaunchBlockPatch> {
    ...
    loop {
        ...
        let skipped = if match_len & 0x8000 > 0 { true } else { false };
        ...
        let block = UnlaunchBlockPatch {
            original_fn: m,
            patch_block_offset: match_off as i16,
            patch_block: p,
            skipped,
        };
        println!("{:x?}", block);
